...
def upgrade():
    ...
    try:
        conn.execute("alter table database_status add column dbs_type varchar(3) not null default 'ANA'")
    except Exception as err:
        print("ERROR add column dbs_type to database_status,\n\terr=" + str(err))

    # ADD COLUMN for catalog reference of a product
    try:
        conn.execute("alter table product_details add column prd_ref_catalog varchar(50) not null default ''")
    except Exception as err:
        print("ERROR add column prd_ref_catalog to product_details,\n\terr=" + str(err))

    # ADD COLUMN for lessor name of a supply product
    try:
        conn.execute("alter table product_supply add column prs_lessor varchar(100) not null default ''")
    except Exception as err:
        print("ERROR add column prs_lessor to product_supply,\n\terr=" + str(err))

    # ADD NEW USER ROLE
    try:
        conn.execute("insert into sigl_pj_role (name, label, type) "
    ...
    try:
        conn.execute("create table requesting_services("
    ...
    try:
        conn.execute("create table functionnal_unit("
    ...
    try:
        conn.execute("create table functionnal_unit_link("
    ...
    try:
        conn.execute("create table stock_setting("
    ...
    try:
        conn.execute("insert into stock_setting "
    ...
    try:
        conn.execute("create table list_comment("
    ...
    try:
        conn.execute("insert into list_comment (lic_date, lic_type, lic_sub_type, lic_ref, lic_user, lic_comm) "
    ...
    try:
        conn.execute("alter table sigl_dico_data drop column dico_id")
    except Exception as err:
        print("ERROR drop column dico_id to sigl_dico_data,\n\terr=" + str(err))

    # DROP COLUMN TO sigl_dico_data TABLE
    try:
        conn.execute("alter table sigl_dico_data drop column dico_value_id")
    except Exception as err:
        print("ERROR drop column dico_value_id to sigl_dico_data,\n\terr=" + str(err))

    # DROP COLUMN TO sigl_dico_data TABLE
    try:
        conn.execute("alter table sigl_dico_data drop column archived")
    except Exception as err:
    ...
    # 1127 => 585
    try:
        conn.execute("update sigl_07_data set type_resultat=585 where type_resultat=1127")

        conn.execute("delete from sigl_dico_data where id_data=1127")
    except Exception as err:
        print("ERROR update type_resultat 1127 to 585 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1128 => 586
    try:
        conn.execute("update sigl_07_data set type_resultat=586 where type_resultat=1128")

        conn.execute("delete from sigl_dico_data where id_data=1128")
    except Exception as err:
        print("ERROR update type_resultat 1128 to 586 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1129 => 588
    try:
        conn.execute("update sigl_07_data set type_resultat=588 where type_resultat=1129")

        conn.execute("delete from sigl_dico_data where id_data=1129")
    except Exception as err:
        print("ERROR update type_resultat 1129 to 588 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1130 => 589
    try:
        conn.execute("update sigl_07_data set type_resultat=589 where type_resultat=1130")

        conn.execute("delete from sigl_dico_data where id_data=1130")
    except Exception as err:
        print("ERROR update type_resultat 1130 to 589 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1137 => 617
    try:
        conn.execute("update sigl_07_data set type_resultat=617 where type_resultat=1137")

        conn.execute("delete from sigl_dico_data where id_data=1137")
    except Exception as err:
        print("ERROR update type_resultat 1137 to 617 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1131 => 591
    try:
        conn.execute("update sigl_07_data set type_resultat=591 where type_resultat=1131")

        conn.execute("delete from sigl_dico_data where id_data=1131")
    except Exception as err:
        print("ERROR update type_resultat 1131 to 591 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1133 => 599
    try:
        conn.execute("update sigl_07_data set type_resultat=599 where type_resultat=1133")

        conn.execute("delete from sigl_dico_data where id_data=1133")
    except Exception as err:
        print("ERROR update type_resultat 1133 to 599 in sigl_07_data then delete in sigl_dico_data,\n\terr=" + str(err))

    # 1134 => 600
    try:
        conn.execute("update sigl_07_data set type_resultat=600 where type_resultat=1134")

        conn.execute("delete from sigl_dico_data where id_data=1134")
    except Exception as err:
