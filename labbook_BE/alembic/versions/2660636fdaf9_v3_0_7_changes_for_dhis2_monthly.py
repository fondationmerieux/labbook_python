...
def upgrade():
    ...
    try:
        conn.execute("update sigl_07_data set type_resultat=246 "
    ...
    try:
        conn.execute("update sigl_15_data set formule='$_236 = [absent.present]' "
    ...
    try:
        conn.execute("update sigl_15_data set label_exp_dhis2='Candida albicans (Prélèvement Urétral)' "
    ...
    try:
        conn.execute("update sigl_07_data set type_resultat=(select id_data from sigl_dico_data "
    ...
    try:
        conn.execute("update sigl_05_data set nom='Test de Détection Rapide (TDR) de recherche de protéines du "
    ...
    try:
        conn.execute("update sigl_05_data set nom='Test sérologique unitaire qualitatif de recherche des anticorps "
    ...
    try:
        conn.execute("update sigl_05_data set nom='Test sérologique quantitatif automatisable de recherche des "
    ...
    try:
        conn.execute("update sigl_05_data set nom='Recherche quantitative du génome du virus SARS-CoV-2 par RT-PCR "
    ...
    try:
        conn.execute("update sigl_05_data set nom='Recherche qualitative du génome du virus SARS-CoV-2 par "
    ...
    try:
        ...
        conn.execute("update sigl_07_data set code_var=id_data where libelle='Phénotype' limit 1")
    except Exception as err:
        print("ERROR insert new variable Phenotype,\n\terr=" + str(err))
    else:
        # Link Phenotype variable to B656, B657, B658, B676, B677, B678
        try:
            conn.execute("insert into sigl_05_07_data (id_owner, id_refanalyse, id_refvariable, position, obligatoire, var_whonet) "
    ...
    try:
        conn.execute("insert into sigl_05_data (id_owner, code, nom, abbr, famille, cote_unite, commentaire, actif, ana_whonet) "
