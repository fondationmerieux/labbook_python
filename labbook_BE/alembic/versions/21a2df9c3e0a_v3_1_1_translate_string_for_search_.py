...
def upgrade():
    ...
    # ADD INDEX
    try:
        conn.execute(text("create index idx_date_samp on sigl_01_data (date_prel)"))

        conn.execute(text("create index idx_date_rec on sigl_02_data (date_dos)"))
        conn.execute(text("create index idx_date_prescr on sigl_02_data (date_prescription)"))
        conn.execute(text("create index idx_date_hosp on sigl_02_data (date_hosp)"))

        conn.execute(text("create index idx_name on sigl_03_data (nom)"))
        conn.execute(text("create index idx_fname on sigl_03_data (prenom)"))

        conn.execute(text("create index idx_name on sigl_05_data (nom)"))

        conn.execute(text("create index idx_label on sigl_07_data (libelle)"))

        conn.execute(text("create index idx_name on sigl_08_data (nom)"))
        conn.execute(text("create index idx_fname on sigl_08_data (prenom)"))
        conn.execute(text("create index idx_code on sigl_08_data (code)"))

        conn.execute(text("create index idx_date_vld on sigl_10_data (date_validation)"))

        conn.execute(text("create index idx_file on sigl_11_data (file)"))

        conn.execute(text("create index idx_label on sigl_dico_data (label)"))

        conn.execute(text("create index idx_oname on sigl_file_data (original_name)"))
        conn.execute(text("create index idx_gname on sigl_file_data (generated_name)"))

        conn.execute(text("create index idx_username on sigl_user_data (username)"))
        conn.execute(text("create index idx_name on sigl_user_data (lastname)"))
        conn.execute(text("create index idx_account on sigl_user_data (side_account)"))

        conn.execute(text("create index idx_date on product_details (prd_date)"))

        conn.execute(text("create index idx_date on product_supply (prs_date)"))
        conn.execute(text("create index idx_user on product_supply (prs_user)"))

        conn.execute(text("create index idx_date on product_use (pru_date)"))
        conn.execute(text("create index idx_user on product_use (pru_user)"))
    except Exception as err:
